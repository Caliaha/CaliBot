...
from stuff import checkPermissions, no_pm
...
class RoleColor():
	...
	async def colorme(self, ctx, *, colorcode: str):
		...
		print(ctx.message.author.id)
		role = discord.utils.get(ctx.message.server.roles, name=colorrole)
		botrole = discord.utils.get(ctx.message.server.roles, name=self.bot.NAME) or discord.utils.get(message.server.roles, name="Calibot")
		if botrole is None:
